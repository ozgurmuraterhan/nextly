...
const Default = ({ getData = [] }) => {
      ...
      <span className="link ant-dropdown-link">
        ...
        {role["topmenu/id"] ?
          <>
            ...
            <Link href={"/topmenu/" + record._id}><a> <EditOutlined style={{ fontSize: "150%", marginLeft: "15px" }} /></a></Link>
          </>
          : ""}
        {role["topmenudelete"] ?
          <>
  ...
  const getDataFc = () => {
    axios.get(API_URL + "/topmenu").then((response) => {
  ...
  const activeOrDeactive = (id, deg) => {
    axios.post(`${API_URL}/topmenu/active/${id}`, { isActive: !deg }).then((res) => {
  ...
  const deleteData = (id) => {
    ...
    axios.delete(`${API_URL}/topmenu/${id}`).then((res) => {
...
Default.getInitialProps = async ({ req }) => {
  if (!req?.headers?.cookie) {
    ...
    const res = await axios.get(API_URL + "/topmenu", { headers: req ? { cookie: req.headers.cookie } : undefined, });
