    ...
    axios
      ...
      .then((res) => {
        if (res.data.variant == "error") {
          message.error(intl.messages["app.pages.customers.notAdded"] + res.data.messagge);
        } else {

          const customersUpdate = res.data

          customersUpdate["created_user"] = { name: res.data.data.name + " " + res.data.data.surname, id: res.data.data._id }
  ...
  return (
    ...
    <div>
      ...
      >
        <Row>
          <Col md={24}>
            <Card className="card" title={intl.messages["app.pages.customers.add"]}>
              <Form.Item
                name="username"
...
Default.getInitialProps = async ({ req, query }) => {
  ...
  if (!req?.headers?.cookie) {
    ...
    const getData = await axios.get(`${API_URL}/turkey`, { headers: req ? { cookie: req.headers.cookie } : undefined, });
