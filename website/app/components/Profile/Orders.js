...
const Defaut = () => {
	...
	return (
		...
		<>
			...
			>
				<Select.Option value="All">{intl.messages["app.pages.orders.all"]}</Select.Option>
				{orderStatus.map(item => (
					<Select.Option ghost key={item._id} value={item._id}>{item.title}</Select.Option>
				))}
			</Select>

			<Radio.Group defaultValue="All" className="float-right mx-auto invisible sm:visible" buttonStyle="solid"

				onChange={(val) => {
					getDataStatusFc(val.target.value);

				}}>
				<Radio.Button value="All">{intl.messages["app.pages.orders.all"]}</Radio.Button>
				{orderStatus.map(item => (
						...
						<div className="m-0 w-full ">
							...
							<table className="w-full bg-black-100  bg-gray-100 !text-center py-5  !rounded-xl ">
								...
								{record.products.map((x, i) =>
									<tr key={i} className="h-20 !border-b !border-black hover:bg-gray-50 ">
								...
								<tr>
									...
									<td className="font-bold">
										...
										<br />
										<Price data={Number(record.total_price) + Number(record.cargo_price)} />
									</td>
								</tr>
							</table>
						</div>,
