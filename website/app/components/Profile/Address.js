...
const Defaut = () => {
   ...
   const [fields, seTfields] = useState(Object.entries(user.address).map(([name, value]) => ({ name, value })));
   ...
   const onSubmitAddress = async (Data) => {
      ...
      if (newAddress.id) {
         ...
         if (isAuthenticated) {
            ...
            await axios.post(`${API_URL}/customerspublic/address`, newAddresArr).then(() => {
               getAddress();
         ...
         axios.post(`${API_URL}/customerspublic/address`, newAddresArr).then(() => {
            setTimeout(() => {
               getAddress();
               seTnewAddress({ open: false, id: null });
            }, 500);
   ...
   return (
      ...
      <div className="w-full">
         <Button
            ...
            onClick={() => {
               seTfields(Object.entries(address[0] ? address[0] : {}).map(([name]) => ({ name, value: null })));
               seTnewAddress({ ...newAddress, open: !newAddress.open });
            }}>
				New Address
         </Button>
         {address && address.map((x, i) =>
            <AddressSelect
               key={i}
               Data={x}
         ...
         <Drawer
            title="Address"
            placement="right"
            onClose={() => { seTnewAddress({ ...newAddress, open: !newAddress.open }); }}
            ...
            >
               ...
               >
                  {selectedO.selectedCountry == "Turkey" ?
                     ...
                     />
                     :
                     <Input placeholder={intl.messages["app.pages.customers.addressTown"]} autocomplete="chrome-off" />
                  }
               </Form.Item>
               ...
               >
                  {selectedO.selectedCountry == "Turkey" ?
                     ...
                     />
                     :
                     <Input placeholder={intl.messages["app.pages.customers.addressDistrict"]} autocomplete="chrome-off" />
                  }
               </Form.Item>
               ...
               >
                  {selectedO.selectedCountry == "Turkey" ?
                     ...
                     />
                     :
                     <Input placeholder={intl.messages["app.pages.customers.addressNeighbour"]} autocomplete="chrome-off" />
                  }
               </Form.Item>
               <Form.Item
                  className="float-left w-full  mx-0 px-0"
               ...
               >
                  <Input.TextArea rows={3} placeholder={intl.messages["app.pages.customers.addressFull"]} autocomplete="chrome-off" />
               </Form.Item>
