  ...
  const onSubmitAddress = async (Data) => {
    ...
    if (newAddress.id) {
      ...
      if (isAuthenticated) {
        ...
        await axios.post(`${API_URL}/customerspublic/address`, newAddresArr).then(res => {
          getAddress()
      ...
      axios.post(`${API_URL}/customerspublic/address`, newAddresArr).then(res => {
        setTimeout(() => {
          getAddress()
          seTnewAddress({ open: false, id: null })
        }, 500);
  ...
  return (
    ...
    <div className="w-full">
      <Button
        ...
        onClick={() => {
          seTfields(Object.entries(address[0] ? address[0] : {}).map(([name, value]) => ({ name, value: null })))
