...
const Page = ({
	...
	const addBasket = (res) => {
		if (basket.length < 1) {
			...
			if (isAuthenticated) {
				axios.post(`${API_URL}/basket/add`, post).then(() => {
					getBasket(user.id);
					...
					.catch(err => {
						message.error({ content: "Some Error, Please Try Again " + err, duration: 3 });
					});
			...
			if (isAuthenticated) {
				...
				axios.post(`${API_URL}/basket/${basket[0]._id}`, post).then(() => {
					getBasket(user.id);
					seTloadingButton(true);
					form.resetFields();
					message.success({ content: "Product Added!", duration: 3 });
