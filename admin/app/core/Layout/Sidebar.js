...
const Sidebar = () => {
    ...
    return (
        <Sider trigger={null} collapsible collapsed={collapsed} style={{ maxWidth: '100%!important', width: "100%", height: '100%', borderRight: "1px solid #cccccc54" }}>
            <div className="sidebarOpenBtn" onClick={() => dispatch(changeCollapsed_r(!collapsed))}   >
                {collapsed ?
                    <MenuUnfoldOutlined />
                    :
                    <MenuFoldOutlined />
            ...
            <Menu theme="dark" mode="inline" className="bg-black" defaultSelectedKeys={['1']}  >
                ...
                {role?.brandsview ?
                    <Menu.Item key="brands" icon={<ChromeOutlined />}>
                        <Link href="/brands/list"><a>
                            <span><IntlMessages id="layout.sidebar.brands" /></span></a></Link>
                    </Menu.Item>
                    : ""}
                ...
                {role?.orderstatusview ?
                    <Menu.Item key="orderstatus" icon={<PartitionOutlined />}>
                        <Link href="/orderstatus/list"><a>
                            <span>  <IntlMessages id="layout.sidebar.orderStatus" /> </span></a></Link>
                    </Menu.Item>
                    : ""}
                {role?.paymentmethodsview ?
                    <Menu.Item key="paymentmethods" icon={<PullRequestOutlined />}>
                        <Link href="/paymentmethods/list"><a>
                ...
                {role?.staffview ?
                    <Menu.Item key="staff" icon={<TeamOutlined />}>
                        <Link href="/staff/list"><a>
                            <span>  <IntlMessages id="layout.sidebar.manager" /> </span></a></Link>
                    </Menu.Item>
                    : ""}
                {role?.superadmin ?
                    <Menu.Item key="settings" icon={<ControlOutlined />}>
                        <Link href="/settings/list"><a>
                            <span>  <IntlMessages id="layout.sidebar.settings" /> </span></a></Link>
                    </Menu.Item>
                    : ""}
                <Menu.Item key="/signin" icon={<CloseSquareOutlined />}
                    onClick={async () => {
                ...
                >
                    <Link href="/signin"><a>
                        <span>  <IntlMessages id="layout.sidebar.logout" /> </span></a></Link>
                </Menu.Item>
            </Menu>
        </Sider>
    );
