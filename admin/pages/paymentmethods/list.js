...
import { Select, message, Image, Table, Popconfirm, Button, Tooltip } from "antd";
...
const Default = ({ getData = [] }) => {
      ...
      <span className="link ant-dropdown-link">
        ...
        {role["paymentmethods/id"] ?
          <>
            ...
            <Link href={"/paymentmethods/" + record._id}><a> <EditOutlined style={{ fontSize: "150%", marginLeft: "15px" }} /></a></Link>
          </>
          : ""}
  ...
  const activeOrDeactive = (id, deg) => {
    axios.post(`${API_URL}/paymentmethods/active/${id}`, { isActive: !deg }).then((res) => {
  ...
  const deleteData = (id, imagePath = 0) => {
    ...
    axios.delete(`${API_URL}/paymentmethods/${id}`).then((res) => {
    ...
    if (imagePath != 0) {
      axios.post(`${API_URL}/upload/deletepaymentmethodsimage`, { path: imagePath }).then((res) => {
        message.success(intl.messages["app.pages.common.deleteData"]);
        getDataFc()
        Router.push("/paymentmethods/list")
...
Default.getInitialProps = async ({ req, query }) => {
  if (!req?.headers?.cookie) {
    ...
    const res = await axios.get(API_URL + "/paymentmethods", { headers: req ? { cookie: req.headers.cookie } : undefined, });
