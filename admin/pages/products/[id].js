...
const Default = ({ getCategories = [], getData = [] }) => {
   ...
   async function getDataFc() {
      await axios.get(`${API_URL}/products/${id}`).then((response) => {

         var output = response.data;
         seTstate(output);
         seTfields(Object.entries(output).map(([name, value]) => ({ name, value })));
   ...
   const selectBeforeVariants = async (dataVariants) => {
      ...
      for (const i in variant) {
         ...
         const dataVariant = dataVariants.filter(x => x.name === variant[i].name);
      ...
      axios
         ...
         .then((res) => {
            if (res.data.variant == "error") {
               message.error(intl.messages["app.pages.product.notUpdated"] + res.data.messagge);
            } else {
   ...
   return (
      ...
      <div>
         ...
         >
            ...
            <Card className="card" title={intl.messages["app.pages.product.edit"]}>
               ...
               >
                  <Select
                     style={{ width: "100%" }}
                     options={[
                        { label: intl.messages["app.pages.product.simple"], value: false },
                        { label: intl.messages["app.pages.product.variant"], value: true }
                     ]}
                     placeholder={intl.messages["app.pages.common.pleaseSelect"]}
                     onChange={(newValue) => {
            ...
            <Card className="card" style={{ display: state.type ? "" : "none" }} title={intl.messages["app.pages.product.productType"]}>
               <Form.List
                  name="variants"
                                    ...
                                    rules={[
                                       ...
                                       ({ getFieldValue }) => ({
                                          validator(rule, value) {
                                             const item = getFieldValue("variants").filter(x => x.name === value);

                                             if (!value || item.length <= 1) {
                                                return Promise.resolve();
                                             }
                                             return Promise.reject(intl.messages["app.pages.common.duplicate"]);
                                          },
                                       }),
                                    ]}
                                 >
                                    ...
                                    <Select
                                       ...
                                       placeholder={intl.messages["app.pages.common.searchVariant"]}
                                       ...
                                       onChange={(selected) => {
                                          const dataVariant = dataVariants.data.filter(x => x.name === selected);
                                          const dataManipulate = [];
                                          const datas = variantsOp.options;
                                 ...
                                 <>
                                    {field.map((field2, j) => (
                                       <div className="float-left" key={j}>
                                          ...
                                          <div className="float-left w-full">
                                             <h5 className="float-left text-xl pr-2"> {j == 0 ? intl.messages["app.pages.common.variants"] : ""} </h5>
                                             {field2.display == true ? "" : <Tag color='blue' className="float-left"  >{field2.initialValue}</Tag>}
                                          </div>
                                          <Form.Item
                                             style={{ display: field2.display == true ? "" : "none" }}
                                             {...field2}
                                             label={field2.label}
                                             {...formItemLayout}
                                             className="float-left w-full  mx-0 px-0"
                                    ...
                                    >
                                       <Select
                                          className=" !w-1/5"
                                          options={[
                                             { label: intl.messages["app.pages.common.beActive"], value: true },
                                             { label: intl.messages["app.pages.common.bePassive"], value: false }
                                          ]}
                                          placeholder={intl.messages["app.pages.common.pleaseSelect"]}
                                       />
...
Default.getInitialProps = async ({ req, query }) => {
   if (!req?.headers?.cookie) {
      ...
      const getData = await axios.get(API_URL + "/products/" + query.id, { headers: req ? { cookie: req.headers.cookie } : undefined, });

      const getDataCategories = await axios.get(`${API_URL}/categories`, { headers: req ? { cookie: req.headers.cookie } : undefined, });
      const geTdataCategoriesManipulate = [];
      if (getDataCategories.data.length > 0) {
         geTdataCategoriesManipulate.push(func.getCategoriesTreeOptions(getDataCategories.data, true));
      }
      return { getData: getData.data, getCategories: geTdataCategoriesManipulate };
   }
