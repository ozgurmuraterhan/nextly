...
const Default = () => {
    ...
    render: (text, record) => (
      <span className="link ant-dropdown-link">
        {role["variantsdelete"] ?
          ...
          : ""}
        {role["variants/id"] ?
          <Link href={"/variants/" + record._id}><a> <EditOutlined style={{ fontSize: "150%", marginLeft: "15px" }} /></a></Link>
          : ""}
      </span>
    ),
  }];
  ...
  const getData = () => {
    axios.get(API_URL + "/variants").then((response) => {
  ...
  const deleteData = (id) => {
    axios.delete(`${API_URL}/variants/${id}`).then((res) => {
