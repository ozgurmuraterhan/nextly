...
import { EditOutlined, DeleteOutlined, CheckCircleOutlined, CloseSquareOutlined, AppstoreAddOutlined } from '@ant-design/icons';
...
const Default = ({ getData = [] }) => {
  ...
  const columns = [{
    ...
    dataIndex: 'name',
    key: 'name',
    render: (text, record) => <span className="link">{record.name} {record.surname}</span>,
  }, {
      ...
      <span className="link ant-dropdown-link">
        ...
        {role["customers/id"] ?
          <div>
            ...
            <Link href={"/customers/" + record._id}><a> <EditOutlined style={{ fontSize: "150%", marginLeft: "15px" }} /></a></Link>
          </div>
          : ""}
      </span>
    ),
  ...
  const activeOrDeactive = (id, deg) => {
    axios.post(`${API_URL}/customers/active/${id}`, { isActive: !deg }).then((res) => {
...
Default.getInitialProps = async ({ req }) => {
  if (!req?.headers?.cookie) {
    ...
    const res = await axios.get(API_URL + "/customers", { headers: req ? { cookie: req.headers.cookie } : undefined, });
