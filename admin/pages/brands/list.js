...
const Default = ({ getData = [] }) => {
      ...
      <span className="link ant-dropdown-link">
        ...
        {role["brands/id"] ?
          <>
            ...
            <Link href={"/brands/" + record._id}><a> <EditOutlined style={{ fontSize: "150%", marginLeft: "15px" }} /></a></Link>
          </>
          : ""}
        {role["brandsdelete"] ?
          <>
  ...
  const activeOrDeactive = (id, deg) => {
    axios.post(`${API_URL}/brands/active/${id}`, { isActive: !deg }).then((res) => {
  ...
  const deleteData = (id, imagePath = 0) => {
    ...
    axios.delete(`${API_URL}/brands/${id}`).then((res) => {
    ...
    if (imagePath != 0) {
      axios.post(`${API_URL}/upload/deletebrandsimage`, { path: imagePath }).then((res) => {
...
Default.getInitialProps = async ({ req, query }) => {
  if (!req?.headers?.cookie) {
    ...
    const res = await axios.get(API_URL + "/brands", { headers: req ? { cookie: req.headers.cookie } : undefined, });
