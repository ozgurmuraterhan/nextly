...
import { Upload, Image, InputNumber, Button, Card, message, Divider, Form, Input } from "antd";
...
const Default = ({ getData = [] }) => {
	...
	const onSubmit = async (Data) => {
		...
		if (Data.image != undefined && state.image != Data.image) {
			...
			const dataImage = await axios.post(`${API_URL}/upload/uploadbrandsimage`, formData, { headers: { "Content-Type": "multipart/form-data" } });
		...
		axios
			...
			.then((res) => {
				if (res.data.variant == "error") {
					message.error(intl.messages["app.pages.brands.brandsNotUpdated"] + res.data.messagge);
				} else {
	...
	return (
		...
		<div>
			<Card className="card" title={intl.messages["app.pages.brands.brandsUpdate"]}>
				<Form
					{...formItemLayout}
					form={form}
					name="add"
...
Default.getInitialProps = async ({ req, query }) => {
	...
	if (!req?.headers?.cookie) {
		...
		const getData = await axios.get(API_URL + "/brands/" + query.id, { headers: req ? { cookie: req.headers.cookie } : undefined, });
