...
const Default = ({ getData = [] }) => {
  ...
  const [cargoesPrice, seTcargoesPrice] = useState(0)
    ...
    axios
      ...
      .then((res) => {
        if (res.data.variant == "error") {
          message.error(intl.messages["app.pages.orders.notUpdated"] + res.data.messagge);
        } else {
                  ...
                  <Button type="primary"
                    ...
                    onClick={() => {
                      ...
                      form.validateFields().then(res => {
                        ...
                        data.forEach(val => {
                          total_price = (Number(val.price) * Number(val.qty)) + total_price
                          discount_price = (Number(val.before_price) * Number(val.qty)) + discount_price
                        });



                        seTstate({ ...state, products: data, total_price, discount_price })
                        seTfields(Object.entries({ ...fields, products: data, total_price, discount_price }).map(([name, value]) => ({ name, value })))
