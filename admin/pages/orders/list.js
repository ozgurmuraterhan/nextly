...
import { Select, message, Table, Popconfirm, Button, Tooltip, Radio } from "antd";
...
const Default = ({ getData = [] }) => {
...

		{
			...
			render: (text, record) => (
				<span className="link ant-dropdown-link">
					...
					{role["orders/id"] ?
						<Link href={"/orders/" + record._id}><a> <EditOutlined style={{ fontSize: "150%", marginLeft: "15px" }} /></a></Link>
						: ""}
					{role["ordersdelete"] ?
						<>
							{record.children ? "" :
	...
	const getDataStatusFc = (target = "All") => {
		...
		axios.get(API_URL + "/orders/status/" + target).then((res) => {
			seTdata(res.data);
	...
	const deleteData = (id, imagePath = 0) => {
		...
		if (imagePath != 0) {
			axios.post(`${API_URL}/upload/deleteproductimage`, { path: imagePath }).then(() => {
				message.success(intl.messages["app.pages.common.deleteData"]);
				getDataFc();
				Router.push("/orders/list");
		...
		<div >
			{role["ordersview"] ?
				<>
					...
					>
						<Select.Option value="All"><IntlMessages id="app.pages.orders.all" /></Select.Option>
						{orderStatus.map(item => (
							<Select.Option ghost key={item._id} value={item._id}>{item.title}</Select.Option>
						))}
					</Select>
					...
					<Radio.Group defaultValue="All" className=" w-full mr-0 h-0 overflow-hidden sm:h-auto sm:overflow-auto  text-center  mx-auto   " buttonStyle="solid"
						...
						<Radio.Button value="All"><IntlMessages id="app.pages.orders.all" /></Radio.Button>
						{orderStatus.map(item => (
							<Radio.Button ghost key={item._id} value={item._id}>{item.title}</Radio.Button>
						))}
					</Radio.Group>
					<Table
						columns={columns}
...
Default.getInitialProps = async ({ req }) => {
	if (!req?.headers?.cookie) {
		...
		const res = await axios.get(API_URL + "/orders", { headers: req ? { cookie: req.headers.cookie } : undefined, });
