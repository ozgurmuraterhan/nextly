	...
	const columns = [{
		...
		render: (text, record) => (
			<span className="link ant-dropdown-link">
				...
				{role["categories/id"] ?
					<Link href={"/categories/" + record._id}><a> <EditOutlined style={{ fontSize: "150%", marginLeft: "15px" }} /></a></Link>
					: ""}
	...
	return (
		...
		<div>
			{role["categories/add"] ?
				...
				: ""}
			<h5> <IntlMessages id="app.pages.category.list" /> </h5>
			<Table
				columns={columns}
...
Default.getInitialProps = async ({ req }) => {
	if (!req?.headers?.cookie) {
		...
		const res = await axios.get(API_URL + "/categories", { headers: req ? { cookie: req.headers.cookie } : undefined, });
