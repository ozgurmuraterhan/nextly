...
import { Switch, TreeSelect, InputNumber, Button, Card, message, Divider, Form, Input } from "antd";
      ...
      axios
         ...
         .then((res) => {
            if (res.data.variant == "error") {
               message.error(intl.messages["app.pages.category.notUpdated"] + res.data.messagge);
            } else {
...
Default.getInitialProps = async ({ req, query }) => {
   if (!req?.headers?.cookie) {
      ...
      const getData = await axios.get(API_URL + "/categories/" + query.id, { headers: req ? { cookie: req.headers.cookie } : undefined, });
