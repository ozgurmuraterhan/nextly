...
const Default = ({ getData = [] }) => {
  ...
  },
  {
    ...
    render: (text, record) => (
      <span className="link ant-dropdown-link">
        ...
        {role["homeslider/id"] ?
          <Link href={"/homeslider/" + record._id}><a> <EditOutlined style={{ fontSize: "150%", marginLeft: "15px" }} /></a></Link>
          : ""}
  ...
  const activeOrDeactive = (id, deg) => {
    axios.post(`${API_URL}/homeslider/active/${id}`, { isActive: !deg }).then((res) => {
  ...
  const getDataFc = () => {
    axios.get(API_URL + "/homeslider").then((response) => {
  ...
  const deleteData = (id) => {
    ...
    axios.delete(`${API_URL}/homeslider/${id}`).then((res) => {
  ...
  return (
    ...
    <div>
      {role["homeslider/add"] ?
        ...
        : ""}
      <h5><IntlMessages id="app.pages.homeSlider.list" /> </h5>
      <Table
        columns={columns}
...
Default.getInitialProps = async ({ req }) => {
  if (!req?.headers?.cookie) {
    ...
    const res = await axios.get(API_URL + "/homeslider", { headers: req ? { cookie: req.headers.cookie } : undefined, });
