...
const Default = ({ getData = [] }) => {
	...
	const onSubmit = async (Data) => {
		...
		if (Data.image != undefined && state.image != Data.image) {
			axios.post(`${API_URL}/upload/deleteorderstatusimage`, { path: state.image });
		...
		axios
			...
			.then((res) => {
				if (res.data.variant == "error") {
					message.error(intl.messages["app.pages.orderStatus.notUpdated"] + res.data.messagge);
				} else {
	...
	return (
		...
		<div>
			<Card className="card" title={intl.messages["app.pages.orderStatus.edit"]}>
				<Form
					{...formItemLayout}
					form={form}
					name="add"
...
Default.getInitialProps = async ({ req, query }) => {
	...
	if (!req?.headers?.cookie) {
		...
		const getData = await axios.get(API_URL + "/orderstatus/" + query.id, { headers: req ? { cookie: req.headers.cookie } : undefined, });
