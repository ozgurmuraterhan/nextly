...
const Default = ({ getData = [] }) => {
  ...
  const columns = [{
    ...
    render: (text, record) => (
      <>
        <Image src={record.image} height={80} />
      </>
    ),
    ...
    render: (text, record) => (
      <span className="link ant-dropdown-link">
        ...
        {role["productimages/id"] ?
          <Link href={"/productimages/" + record._id}><a> <EditOutlined style={{ fontSize: "150%", marginLeft: "15px" }} /></a></Link>
          : ""}
        {role["productimagesdelete"] ?
          <>
            {record.children ? "" :
              <Popconfirm title={intl.messages["app.pages.common.youSure"]} onConfirm={() => deleteData(record._id, record.image, id)}>
  ...
  const getDataFc = (id) => {
    axios.get(API_URL + "/productimages").then((res) => {
  ...
  const deleteData = (id, imagePath = 0, quertId) => {
    ...
    axios.delete(`${API_URL}/productimages/${id}`).then((res) => {
  ...
  return (
    <div>
      <IntlMessages id="app.pages.productimages.product" /> : <Select
        defaultValue={id}
...
Default.getInitialProps = async ({ req, query }) => {
  if (!req?.headers?.cookie) {
    ...
    const res = await axios.get(API_URL + "/productimages", { headers: req ? { cookie: req.headers.cookie } : undefined, });

    const dataManipulate = res.data

    if (query.id) {
      return { getData: dataManipulate.filter(x => x.product_id._id == query.id) }
    } else {
