...
const Default = ({ getProducts = [], getData = [] }) => {
      ...
      axios
         ...
         .then((res) => {
            if (res.data.variant == "error") {
               message.error(intl.messages["app.pages.productimages.notUpdated"] + res.data.messagge);
            } else {
   ...
   return (
      ...
      <div>
         <Card className="card" title={intl.messages["app.pages.productimages.update"]}>
            <Form
               {...formItemLayout}
               form={form}
               name="add"
...
Default.getInitialProps = async ({ req, query }) => {
   ...
   if (!req?.headers?.cookie) {
      ...
      const getData = await axios.get(API_URL + "/productimages/" + query.id, { headers: req ? { cookie: req.headers.cookie } : undefined, });
      ...
      return { getData: geTdataManipulate, getProducts: getDataProductsManipulate };
   }
