...
import { Upload, Image, InputNumber, Button, Card, message, Divider, Form, Input, Select, } from "antd";
...
const Default = ({ getData = [] }) => {
	...
	const onSubmit = async (Data) => {
		...
		if (Data.image != undefined && state.image != Data.image) {
			...
			const dataImage = await axios.post(`${API_URL}/upload/uploadcargoimage`, formData, { headers: { "Content-Type": "multipart/form-data" } });
		...
		axios
			...
			.then((res) => {
				if (res.data.variant == "error") {
					message.error(intl.messages["app.pages.cargoes.notUpdated"] + res.data.messagge);
				} else {
	...
	return (
		...
		<div>
			<Card className="card" title={intl.messages["app.pages.cargoes.cargoCompanyEdit"]}>
				<Form
					{...formItemLayout}
					form={form}
					name="add"
				...
				>
					...
					>
						<Upload maxCount={1}
							...
							showUploadList={{
								removeIcon: <DeleteOutlined onClick={() => seTdisplaySave(true)} />,
							}}
						>
...
Default.getInitialProps = async ({ req, query }) => {
	...
	if (!req?.headers?.cookie) {
		...
		const getData = await axios.get(API_URL + "/cargoes/" + query.id, { headers: req ? { cookie: req.headers.cookie } : undefined, });
