...
const Default = ({ getData = [] }) => {
  ...
  },
  {
    ...
    render: (text, record) => (
      <span className="link ant-dropdown-link">
        ...
        {role["cargoes/id"] ?
          <Link href={"/cargoes/" + record._id}><a> <EditOutlined style={{ fontSize: "150%", marginLeft: "15px" }} /></a></Link>
          : ""}
        {role["cargoesdelete"] ?
          <>
            {record.children ? "" :
  ...
  const deleteData = (id, imagePath = 0) => {
    ...
    axios.delete(`${API_URL}/cargoes/${id}`).then((res) => {
...
Default.getInitialProps = async ({ req, query }) => {
  if (!req?.headers?.cookie) {
    ...
    const res = await axios.get(API_URL + "/cargoes", { headers: req ? { cookie: req.headers.cookie } : undefined, });
