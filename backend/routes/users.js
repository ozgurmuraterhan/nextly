...
router.post("/loginuser", passport.authenticate("local", { session: false }), (req, res) => {
    if (req.isAuthenticated()) {
        ...
        res.status(200).json({
            ...
            user: { username, id: _id, name: name, surname, company, isCustomer, image, address, isActive, prefix, phone },
        });
    }
}
);
...
router.post("/login", passport.authenticate("local", { session: false }), (req, res) => {
    if (req.isAuthenticated()) {
        const { _id, username, role, name, surname, company, isCustomer, image, phone } = req.user;
        const token = signToken(_id);
        res.cookie("access_token", token, {
            httpOnly: true,
            sameSite: true,
        ...
        res.status(200).json({
            isAuthenticated: true,
            user: { username, role, id: _id, name: name + " " + surname, company: company, isCustomer: isCustomer, image: image, phone: phone },
        });
    }
}
);
...
router.put("/updatePasswordViaEmail", (req, res) => {
    Users.findOne({
        ...
        if (user == null) {
            ...
            res.status(200).send({ message: "password reset link is invalid or has expired" });
        } else if (user != null) {
            bcrypt
...
router.get(
    ...
    (req, res) => {
        ...
        res.json({
            user: { username: "", role: "", id: "", name: "", company: "", isCustomer: "", phone: "" },
            success: true,
        });
    }
);
...
router.get(
    ...
    (req, res) => {
        ...
        res.status(200).json({
            isAuthenticated: true,
            user: { username, role, id: _id, name: name, surname, company: company, isCustomer: isCustomer, image: image, prefix: prefix, phone: phone },
        });
    }
);
