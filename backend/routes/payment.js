...
const getSaveProductsBaskettoOrders = async (data = [], products = [], allBasket = []) => {
    ...
    allBasket[0].shipping_address = allBasket[0].shipping_address.address + " " + allBasket[0].shipping_address.village_id + " " + allBasket[0].shipping_address.district_id + " " + allBasket[0].shipping_address.town_id + " " + allBasket[0].shipping_address.city_id
    allBasket[0].billing_address = allBasket[0].billing_address.address + " " + allBasket[0].billing_address.village_id + " " + allBasket[0].billing_address.district_id + " " + allBasket[0].billing_address.town_id + " " + allBasket[0].billing_address.city_id
    ...
    Paymentmethods.findById("6132787ae4c2740b7aff7320")
        .then(async resPay => {
            if (resPay.secret_key != "" && req.body.items.length > 0) {
                ...
                return await { basket: req.body.allBasket, items: items, ids: ids, clientSecret: paymentIntent.client_secret }
            }
...
router.route('/stripeokey').post(async (req, res, next) => {
...
router.route('/stripeconfirm/:payment_intent/:ordernumber').get((req, res, next) => {
